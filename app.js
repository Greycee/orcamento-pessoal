...
class Bancodados{
  ...
  recuperarTodosRegistros(){
    ...
    for(let i = 1; i <= id; i++){ 
      ...
      //se sim, pular estes itens e não incluí-los no array
      if(despesa == null){
        continue
      }
      despesa.id = i
...
function cadastrarDespesa(){
  ...
  if(despesa.validarDados()){
    ...
    corTitulo.className = 'modal-header text-success'
    modalTitulo.innerText = 'Registro inserido com sucesso'
    modalMensagem.innerText = 'Despesa cadastrada com sucesso!'
    modalBotao.className = 'btn btn-success'
    modalBotao.innerText = 'Ok'
    $('#modal').modal('show')
    ano.value = '' 
    mes.value = ''
    dia.value = ''
    ...
    corTitulo.className = 'modal-header text-danger'
    modalTitulo.innerText = 'Erro na gravação'
    modalMensagem.innerText = 'Existem campos obrigatórios que não foram preenchidos.'
    modalBotao.className = 'btn btn-danger'
    modalBotao.innerText = 'Voltar e corrigir'
    $('#modal').modal('show')
  }
}
